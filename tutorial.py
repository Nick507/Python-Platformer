...
def handle_player_move(player, objects):
    ...
    player.x_vel = 0
    if keys[pygame.K_LEFT]: player.move_left(Player.VELOCITY)
    if keys[pygame.K_RIGHT]: player.move_right(Player.VELOCITY)
...
def handleEvents(objects, player):
    ...
    if currentObject != -1:
        keys = pygame.key.get_pressed()
        if keys[pygame.K_LEFT]: offset_x -= 20
        if keys[pygame.K_RIGHT]: offset_x += 20
